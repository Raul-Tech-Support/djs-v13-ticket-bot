...
module.exports = async (client, interaction) => {
	...
	if (interaction.isButton()) {
		...
		if (!button) {
			try {
				return await interaction.reply({ embeds: [buttonNotFoundEmbed], components: [reportButton], ephemeral: true });
			}
			catch {
				return await interaction.followUp({ embeds: [buttonNotFoundEmbed], components: [reportButton], ephemeral: true });
			}
		}
		...
		try {
			...
			console.error(`[${botName}] There was an error when trying to execute the button: ${interaction.customId}`, error);
